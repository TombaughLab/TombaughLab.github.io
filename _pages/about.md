...
Breve Resumen: 
<p style="text-align: justify;">La teoría del funcional de la densidad (Density Functional Theory, DFT) surge como una alternativa al tratamiento directo del problema cuántico de muchos electrones. En un sistema formado por $N$ electrones bajo el potencial de los núcleos, la descripción completa está dada por la ecuación de Schrödinger estacionaria $ \hat H \Psi(\mathbf r_1,\mathbf r_2,\dots,\mathbf r_N) = E \Psi(\mathbf r_1,\mathbf r_2,\dots,\mathbf r_N) $, donde el Hamiltoniano electrónico puede escribirse como $ \hat H = \sum_{i=1}^{N}\left(-\frac{\hbar^2}{2m}\nabla_i^2 + V(\mathbf r_i)\right) + \sum_{i<j}\frac{e^2}{|\mathbf r_i-\mathbf r_j|} $. La función de onda $\Psi$ depende de $3N$ coordenadas espaciales, lo que provoca que la complejidad del problema crezca exponencialmente con el número de electrones y se vuelva prácticamente intratable para sistemas grandes. La idea fundamental de DFT consiste en reformular este problema reemplazando la función de onda por la densidad electrónica $ n(\mathbf r) = N \int |\Psi(\mathbf r,\mathbf r_2,\dots,\mathbf r_N)|^2 \, d\mathbf r_2 \dots d\mathbf r_N $, una cantidad que depende únicamente de tres coordenadas espaciales y que, según los teoremas de Hohenberg y Kohn, determina de manera única el potencial externo y por tanto todas las propiedades físicas del sistema.</p>
</p>

<p style="text-align: justify;">
En este marco la energía total del sistema puede escribirse como un funcional de la densidad $ E[n] = T[n] + U[n] + \int V(\mathbf r)\, n(\mathbf r)\, d^3r $, donde $T[n]$ representa la energía cinética electrónica y $U[n]$ la interacción entre electrones. El estado fundamental se obtiene minimizando este funcional respecto a todas las densidades físicamente admisibles. En la práctica la formulación más utilizada es la propuesta por Kohn y Sham, que introduce un sistema ficticio de electrones no interactuantes cuya densidad reproduce exactamente la del sistema real. Esto conduce a las ecuaciones $ \left[-\frac{\hbar^2}{2m}\nabla^2 + V_{\text{eff}}(\mathbf r)\right]\varphi_i(\mathbf r) = \varepsilon_i \varphi_i(\mathbf r) $, a partir de las cuales la densidad se reconstruye como $ n(\mathbf r) = \sum_i |\varphi_i(\mathbf r)|^2 $. El potencial efectivo incluye el potencial externo, el término de Hartree y el funcional de intercambio–correlación $V_{\text{xc}}$, que incorpora los efectos cuánticos de interacción electrónica. La determinación aproximada de este término constituye el principal desafío de la teoría, pero también la razón de su enorme éxito como herramienta computacional para estudiar la estructura electrónica de átomos, moléculas, sólidos y nanoestructuras.
</p>
...
<div style="text-align:center;">
<iframe width="720" height="405"
src="https://www.youtube.com/embed/iHYPzZcLv_Q"
title="Introduction to Raman Spectroscopy"
frameborder="0"
allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
allowfullscreen>
</iframe>
